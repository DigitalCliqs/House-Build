// Reference-led production material pass for the Anamarija next-gen shell.
// Remote CC0 maps enhance the base materials when reachable; failed texture loads preserve tuned procedural PBR rather than degrading the scene.
export function applyProductionMaterialPass({THREE,scene,renderer}={}){
 ...
 return Promise.all([
  physical(sets.marble,{color:0xf4f0e9,roughness:.19,normalScale:.18,clearcoat:.38,clearcoatRoughness:.16,envMapIntensity:1.35}),
  physical(sets.oak,{color:0xc18e61,roughness:.43,normalScale:.28,clearcoat:.06,clearcoatRoughness:.38,envMapIntensity:.75}),
  physical(sets.roof,{color:0x3b3d40,roughness:.68,normalScale:.42,envMapIntensity:.65,side:THREE.DoubleSide}),
  physical(sets.stone,{color:0xd7d0c5,roughness:.58,normalScale:.28,envMapIntensity:.65})
  ...
  return{materials,loadedTextureSets:loaded,totalTextureSets:materials.length,dispose(){owned.forEach(t=>t.dispose());materials.forEach(m=>m.dispose())}};
 });
}
