...
if (window.__HOUSE_IS_MOBILE__) {
  ...
  style.textContent = `
    ...
    .mobileButtons { position:absolute; right:14px; bottom:max(22px,env(safe-area-inset-bottom)); display:grid; gap:8px; pointer-events:auto; }
  ...
  function key(code, down) {
    if (down && !held.has(code)) { held.add(code); dispatchEvent(new KeyboardEvent('keydown',{code,key:code,bubbles:true})); }
    else if (!down && held.has(code)) { held.delete(code); dispatchEvent(new KeyboardEvent('keyup',{code,key:code,bubbles:true})); }
  }
  ...
  function setJoy(clientX, clientY){
    const r=joy.getBoundingClientRect(); let dx=clientX-(r.left+r.width/2), dy=clientY-(r.top+r.height/2);
    const max=r.width*.34, len=Math.hypot(dx,dy)||1; if(len>max){dx*=max/len;dy*=max/len;}
    knob.style.transform=`translate(${dx}px,${dy}px)`; const nx=dx/max, ny=dy/max, dead=.24;
  ...
  look.addEventListener('pointermove',e=>{if(e.pointerId!==lookPointer)return;const dx=e.clientX-lastX,dy=e.clientY-lastY;lastX=e.clientX;lastY=e.clientY;const ev=new MouseEvent('mousemove',{bubbles:true});Object.defineProperty(ev,'movementX',{value:dx*1.15});Object.defineProperty(ev,'movementY',{value:dy*1.15});document.dispatchEvent(ev);});
  ...
  document.getElementById('mPanel').onclick=()=>{
    ...
    document.body.classList.add('menu-open'); p.style.display='block'; p.scrollTop=0;
  };
