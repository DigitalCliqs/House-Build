...
export function createTerracePoolV42({THREE,scene}={}){
 ...
 // Large-format terrace/pool paving visual joints; all geometry remains flush.
 ...
 // Premium outdoor lounge group positioned away from the 1.5 m centre route.
 ...
 // Pool coping and inset water glow visually sharpen the 8 x 4 m pool.
 ...
 // Olive-style clustered planting and ornamental grasses around the rear boundary.
 ...
 for(const [i,[x,z]] of grass.entries()){
  for(let j=0;j<7;j++){const h=.45+(j%3)*.09;box(`v42:grass-${i}-${j}`,[x+(j-3)*.055,h/2,z+(j%2)*.07],[.025,h,.025],j%2?green2:green1,false)}
 }
 // Warm concealed terrace wash toward planting, not the accessible route.
